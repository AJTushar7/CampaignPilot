...
export class DashboardComponent implements OnInit, OnDestroy {
  ...
  private applyChannelFilter(): void {
    // Update KPI cards based on selected channel
    this.updateKPICards();
    // Update channel performance data
    this.updateChannelPerformance();
    // Update weekly campaign overview
    this.updateWeeklyCampaignData();
  }

  private updateKPICards(): void {
    // Filter and update KPI data based on selected channel
    if (this.selectedChannel === 'all') {
      // Show aggregated data for all channels
      this.totalCampaigns = 148;
      this.activeCampaigns = 12;
      this.totalSpend = 125000;
    } else if (this.selectedChannel === 'sms') {
      this.totalCampaigns = 45;
      this.activeCampaigns = 8;
      this.totalSpend = 45000;
    } else if (this.selectedChannel === 'whatsapp') {
      this.totalCampaigns = 38;
      this.activeCampaigns = 6;
      this.totalSpend = 32000;
    } else if (this.selectedChannel === 'email') {
      this.totalCampaigns = 35;
      this.activeCampaigns = 4;
      this.totalSpend = 28000;
    } else if (this.selectedChannel === 'push') {
      this.totalCampaigns = 30;
      this.activeCampaigns = 3;
      this.totalSpend = 20000;
    }
  }
  ...
  private updateChannelPerformance(): void {
    ...
    if (this.selectedChannel === 'all') {
      this.channelPerformanceData = originalChannelData;
    } else {
      // Show single channel data
      const channelData = originalChannelData.find(c => c.name.toLowerCase() === this.selectedChannel);
      if (channelData) {
        this.channelPerformanceData = [{ ...channelData, percentage: 100 }];
      }
    }
  }

  private updateWeeklyCampaignData(): void {
    // Update weekly campaign overview based on channel filter
    if (this.selectedChannel === 'all') {
      this.totalActiveCampaigns = 14;
      this.totalScheduledCampaigns = 19;
      this.totalWeeklyCampaigns = 52;
    } else if (this.selectedChannel === 'sms') {
      this.totalActiveCampaigns = 6;
      this.totalScheduledCampaigns = 8;
      this.totalWeeklyCampaigns = 22;
    } else if (this.selectedChannel === 'whatsapp') {
      this.totalActiveCampaigns = 4;
      this.totalScheduledCampaigns = 6;
      this.totalWeeklyCampaigns = 18;
    } else if (this.selectedChannel === 'email') {
      this.totalActiveCampaigns = 3;
      this.totalScheduledCampaigns = 4;
      this.totalWeeklyCampaigns = 12;
    } else if (this.selectedChannel === 'push') {
      this.totalActiveCampaigns = 1;
      this.totalScheduledCampaigns = 1;
      this.totalWeeklyCampaigns = 8;
    }
  }
